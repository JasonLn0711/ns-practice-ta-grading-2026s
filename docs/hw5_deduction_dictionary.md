...
| `ACCURACY_BELOW_TARGET` | MNIST test accuracy is below `98%`. | 1-15 | yes if near threshold | `Reported MNIST test accuracy <value> is below 98%.` |
...
| `WEAK_AUDIT_EVIDENCE` | Code/results are inspectable but logs, configuration trace, or retained outputs are incomplete. | 1-5 | no | `Evidence is weak because <observable issue>.` |
