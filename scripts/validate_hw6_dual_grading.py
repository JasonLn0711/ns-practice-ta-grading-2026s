...
CODE_FIELDS = [
    ...
    "graph_consistent_code_score", "training_algorithm_score",
    "result_quality_score", "evidence_auditability_score",
    "code_penalty_score", "code_total_score", "evidence_level",
    "manual_review_needed", "code_deduction_tags", "grader", "graded_at",
    "notes",
]
FIGURE_FIELDS = [
    "student_name", "student_id", "submission_path", "computational_graph_score",
    "learned_filters_score", "feature_maps_score", "figure_explainability_score",
    "figure_penalty_score", "figure_total_score", "evidence_level",
...
CODE_MAX = {
    ...
    "graph_consistent_code_score": 20,
    "training_algorithm_score": 25,
    "result_quality_score": 20,
    "evidence_auditability_score": 10,
}
FIGURE_MAX = {
    "computational_graph_score": 40,
    "learned_filters_score": 20,
    "feature_maps_score": 25,
    "figure_explainability_score": 15,
}
...
def main() -> int:
    ...
    for row in code_rows:
        sid = row["student_id"]
        subtotal = sum(number(row, field) for field in CODE_MAX) - number(row, "code_penalty_score")
        if subtotal != number(row, "code_total_score"):
        ...
        elif number(row, "evidence_auditability_score") > CODE_EVIDENCE_CAP[level]:
        ...
        if number(row, "code_total_score") < 100 and not tag_set(row["code_deduction_tags"]):
        ...
        note_path = HW6 / "student_notes" / f"{sid}.md"
        if not note_path.exists():
            errors.append(f"{sid}: missing student note")
        else:
            note_text = note_path.read_text(encoding="utf-8")
            for needle in ["HW6 Dual-Score Revision", "HW6(code)", "HW6(figure)"]:
                if needle not in note_text:
                    errors.append(f"{sid}: note missing {needle}")

    for row in figure_rows:
        sid = row["student_id"]
        subtotal = sum(number(row, field) for field in FIGURE_MAX) - number(row, "figure_penalty_score")
        if subtotal != number(row, "figure_total_score"):
        ...
        elif number(row, "figure_explainability_score") > FIGURE_EVIDENCE_CAP[level]:
        ...
        if number(row, "figure_total_score") < 100 and not tag_set(row["figure_deduction_tags"]):
    ...
    for row in combined_rows:
        ...
        if sid not in code_by_id or sid not in figure_by_id:
            ...
            continue
        if number(row, "hw6_code_score") != number(code_by_id[sid], "code_total_score"):
            errors.append(f"{sid}: combined code score mismatch")
        if number(row, "hw6_figure_score") != number(figure_by_id[sid], "figure_total_score"):
