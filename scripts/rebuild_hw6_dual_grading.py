...
Running it rewrites the dual-score CSVs and appends/replaces the dual-score
section in each existing per-student note.
"""
...
ROWS = [
    ...
    row("113304030", "鄭丞言", "submissions/hw6/renamed/hw6_113304030_01_hw6.ipynb", (25, 0, 25, 20, 7), (0, 0, 22, 4), "B", "C", "CODE_GRAPH_NOTATION_MISMATCH", "FIG_GRAPH_MISSING;FIG_FILTERS_MISSING;FIG_FEATURE_MAPS_UNCLEAR;FIG_LABELING_INSUFFICIENT", "Strong code/result evidence, but no computational graph artifact exists, so graph-consistent implementation receives zero and Level B code evidence applies.", "Feature maps are present, but the computational graph and learned-filter plot are missing; figure evidence is weak.", [("CODE_GRAPH_NOTATION_MISMATCH", "graph_consistent_code_score", 20, "No computational graph artifact was found, so code/graph notation alignment cannot be verified."), ("CODE_GRAPH_NOTATION_MISMATCH", "evidence_auditability_score", 3, "Missing graph prevents full code/graph/result traceability.")], [("FIG_GRAPH_MISSING", "computational_graph_score", 40, "No computational graph artifact was found."), ("FIG_FILTERS_MISSING", "learned_filters_score", 20, "No learned-filter plot tied to trained filters was found."), ("FIG_FEATURE_MAPS_UNCLEAR", "feature_maps_score", 3, "Feature maps are present but not enough to compensate for missing graph/filter figure evidence."), ("FIG_LABELING_INSUFFICIENT", "figure_explainability_score", 11, "Only partial feature-map figure evidence is auditable.")]),
    ...
    row("214952033", "陳奕樺", "submissions/hw6/renamed/hw6_214952033_01_hw6.ipynb", (10, 0, 12, 20, 7), (0, 20, 25, 8), "B", "B", "CODE_ARCH_REPLACEMENT_INCOMPLETE;CODE_GRAPH_NOTATION_MISMATCH;CODE_MOMENTUM_MISSING", "FIG_GRAPH_MISSING;FIG_LABELING_INSUFFICIENT", "Code has result, filters, and maps, but architecture lacks the required added hidden layers, graph is missing, and Adam is used instead of explicit momentum.", "Filters and feature maps are present, but the computational graph is missing.", [("CODE_ARCH_REPLACEMENT_INCOMPLETE", "architecture_score", 15, "Submitted model has one hidden fully connected layer plus output rather than two added hidden layers."), ("CODE_GRAPH_NOTATION_MISMATCH", "graph_consistent_code_score", 20, "No computational graph artifact was found."), ("CODE_MOMENTUM_MISSING", "training_algorithm_score", 13, "Training uses Adam rather than mini-batch stochastic gradient with historical momentum accumulation."), ("CODE_GRAPH_NOTATION_MISMATCH", "evidence_auditability_score", 3, "Missing graph and missing required momentum evidence cap code auditability.")], [("FIG_GRAPH_MISSING", "computational_graph_score", 40, "No computational graph artifact was found."), ("FIG_LABELING_INSUFFICIENT", "figure_explainability_score", 7, "Figures are partially auditable but graph traceability is absent.")]),
    ...
    row("314261002", "洪至寬", "submissions/hw6/renamed/hw6_314261002_01_hw6.ipynb", (25, 0, 25, 18, 7), (0, 20, 25, 8), "B", "B", "CODE_GRAPH_NOTATION_MISMATCH;CODE_ACCURACY_BELOW_TARGET", "FIG_GRAPH_MISSING;FIG_LABELING_INSUFFICIENT", "Architecture and momentum are strong, but no graph artifact exists and retained test accuracy is 97.62%.", "Filters and feature maps are present, but computational graph evidence is missing.", [("CODE_GRAPH_NOTATION_MISMATCH", "graph_consistent_code_score", 20, "No computational graph artifact was found."), ("CODE_ACCURACY_BELOW_TARGET", "result_quality_score", 2, "Retained test accuracy is 97.62%, below the 98% target."), ("CODE_GRAPH_NOTATION_MISMATCH", "evidence_auditability_score", 3, "Missing graph prevents full code/graph/result traceability.")], [("FIG_GRAPH_MISSING", "computational_graph_score", 40, "No computational graph artifact was found."), ("FIG_LABELING_INSUFFICIENT", "figure_explainability_score", 7, "Visualization evidence is present, but graph traceability is absent.")]),
    ...
    row("314264001", "陳耘加", "submissions/hw6/renamed/hw6_314264001_01_hw6-314264001.ipynb", (25, 0, 25, 20, 7), (0, 20, 25, 8), "B", "B", "CODE_GRAPH_NOTATION_MISMATCH", "FIG_GRAPH_MISSING;FIG_LABELING_INSUFFICIENT", "Code is strong and target-reaching, but graph-consistent implementation cannot be verified because graph evidence is missing.", "Filters and feature maps are present, but no graph artifact exists.", [("CODE_GRAPH_NOTATION_MISMATCH", "graph_consistent_code_score", 20, "No computational graph artifact was found."), ("CODE_GRAPH_NOTATION_MISMATCH", "evidence_auditability_score", 3, "Missing graph caps code auditability.")], [("FIG_GRAPH_MISSING", "computational_graph_score", 40, "No computational graph artifact was found."), ("FIG_LABELING_INSUFFICIENT", "figure_explainability_score", 7, "Visualization evidence is present, but graph traceability is absent.")]),
    ...
    row("314351006", "陳叡儀", "submissions/hw6/renamed", (22, 12, 25, 20, 7), (25, 20, 25, 10), "B", "B", "CODE_ARCH_REPLACEMENT_INCOMPLETE;CODE_GRAPH_NOTATION_MISMATCH", "FIG_GRAPH_CODE_MISMATCH;FIG_LABELING_INSUFFICIENT", "Graph shows three hidden layers, but code uses two hidden fully connected layers before output.", "Graph is visible but does not fully match code hidden-layer count.", [("CODE_ARCH_REPLACEMENT_INCOMPLETE", "architecture_score", 3, "Notebook code implements two hidden fully connected layers while current rubric expects two additional hidden layers."), ("CODE_GRAPH_NOTATION_MISMATCH", "graph_consistent_code_score", 8, "Submitted graph and code disagree on hidden-layer count."), ("CODE_GRAPH_NOTATION_MISMATCH", "evidence_auditability_score", 3, "Graph/code mismatch caps code auditability.")], [("FIG_GRAPH_CODE_MISMATCH", "computational_graph_score", 15, "Graph shows a different hidden-layer count from the submitted code."), ("FIG_LABELING_INSUFFICIENT", "figure_explainability_score", 5, "Graph/code mismatch caps figure explainability.")]),
    ...
    row("514661019", "蔡宇量", "submissions/hw6/renamed/hw6_514661019_01_homework-6-514661019.ipynb", (25, 18, 25, 20, 10), (30, 20, 25, 10), "A", "B", "CODE_GRAPH_NOTATION_MISMATCH", "FIG_GRAPH_INCOMPLETE;FIG_LABELING_INSUFFICIENT", "Code is complete and graph notation is aligned, but graph evidence appears only as source comments.", "Source-comment graph notation is model-specific, but not a rendered slide-style graph artifact.", [("CODE_GRAPH_NOTATION_MISMATCH", "graph_consistent_code_score", 2, "Graph notation is source comments only, so implementation/graph consistency is less independently auditable.")], [("FIG_GRAPH_INCOMPLETE", "computational_graph_score", 10, "Graph evidence appears as source comments rather than a rendered slide-style graph artifact."), ("FIG_LABELING_INSUFFICIENT", "figure_explainability_score", 5, "Source-comment graph format caps figure explainability.")]),
]
...
def update_note(item: dict[str, object]) -> None:
    note_path = HW6 / "student_notes" / f"{item['student_id']}.md"
...
- Combined evidence level: `{combined_level(item['code_level'], item['figure_level'])}`
...
| --- | ---: | ---: | --- |
| Architecture implementation correctness | {code[0]} | 25 | {item['code_note']} |
| Computational-graph-consistent implementation | {code[1]} | 20 | {item['code_note']} |
| Training algorithm correctness | {code[2]} | 25 | {item['code_note']} |
| Result quality | {code[3]} | 20 | {item['code_note']} |
| Evidence and auditability | {code[4]} | 10 | evidence level `{item['code_level']}` |
...
| Category | Score | Max | Evidence note |
| --- | ---: | ---: | --- |
| Computational graph correctness and completeness | {figure[0]} | 40 | {item['figure_note']} |
| Learned filters visualization | {figure[1]} | 20 | {item['figure_note']} |
| Intermediate feature maps visualization | {figure[2]} | 25 | {item['figure_note']} |
| Figure evidence and explainability | {figure[3]} | 15 | evidence level `{item['figure_level']}` |
...
TA audit summary: {item['code_note']} {item['figure_note']} Every non-full
category above is represented in the dual-score deduction logs.
{END}
""".strip()
    note_path.write_text(original.rstrip() + "\n\n" + section + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    for item in ROWS:
        ...
        code_rows.append({
            ...
            "graph_consistent_code_score": code[1],
            "training_algorithm_score": code[2],
            "result_quality_score": code[3],
            "evidence_auditability_score": code[4],
            "code_penalty_score": 0,
            "code_total_score": code_total,
        ...
        figure_rows.append({
            ...
            "student_id": item["student_id"],
            "submission_path": item["submission_path"],
            "computational_graph_score": figure[0],
            "learned_filters_score": figure[1],
            "feature_maps_score": figure[2],
            "figure_explainability_score": figure[3],
            "figure_penalty_score": 0,
            "figure_total_score": figure_total,
